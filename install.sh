...
print_banner() {
    ...
    echo " |_| \_| \_____| |_|   /_/    \_\_|  \_/_/    \_\____"          "
    echo "                                                          "
    echo "            NC PANEL MANAGEMENT & INSTALLER              "
    echo "            Main Panel Default Port: 1234                "
    echo "  ========================================================"
    echo -e "${NC}"
}
...
install_panel() {
    ...
    if [ -f "package.json" ] && grep -q "react-example" "package.json" 2>/dev/null; then
        ...
        log_info "The 'Nc' folder already exists. Running setup inside it..."
        WORK_DIR="Nc"
    else
...
npx pm2 restart nc-panel || npx pm2 restart all
...
if [ -f "package.json" ] && grep -q "react-example" "package.json" 2>/dev/null; then
    ...
    log_error "'NC' directory not found!"
    return
fi
...
restart_panel() {
print_banner
echo -e "${BOLD}--- [4] Restart NC Panel ---${NC}\n"
...
while true; do
print_banner
echo -e "  ${BOLD}1)${NC} Install Panel (Auto Setup - Port 1234)"

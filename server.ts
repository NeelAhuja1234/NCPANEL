...
io.use((socket, next) => {
  ...
  try {
    const verified = nc.verify(token, process.env.NC_SECRET || "nc-panel-super-secret");
...
const PORT = process.env.PORT ? parseInt(process.env.PORT, 10) : (isDev ? 5678 : 1234);
...
import { initSFTPServer } from "./src/server/services/sftp.js"1;
